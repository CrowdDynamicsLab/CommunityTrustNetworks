    ...
    for idx_t, (Tau_a, Tau_b) in enumerate(Tau_list):
        for sim_it in range(sim_iters):
            ...
            for i in range(ntwk_iters+extra_iters):
                ...
                num_accepted_prop += formation.christakis(G, recs, i)

            #tri = metrics.triangles(G)

            #trust_list = [node[1]['trust'] for node in G.nodes.data()]
            results_arr1[idx_r][idx_t][sim_it] = num_accepted_prop
            results_arr2[idx_r][idx_t][sim_it] = num_total_prop
...
plotting.heat_map(results_arr1, new_rho_list, Tau_list, type = 'num', title = 'rho_vs_tau_num_rec_accepted', save = True)
plotting.heat_map(results_arr2, new_rho_list, Tau_list, type = 'num', title = 'rho_vs_tau_num_total_recs', save = True)
plotting.heat_map(results_arr3, new_rho_list, Tau_list, type = 'num', title = 'rho_vs_tau_num_none_recs', save = True)
